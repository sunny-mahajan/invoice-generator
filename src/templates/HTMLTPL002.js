...
export default function generateHTMLTPL002(invoiceData) {
  ...
  const currencySymbol = (currency) => {
    ...
    return currencySymbols[currency] || "₹"; // Default to INR if not found
  };
  ...
  <div class="container">
    ...
    <div class="header">
      ...
      ${
        invoiceData["Logo"]
          ? `<div style="margin-bottom: 80px;">
          <img src="${invoiceData["Logo"]}" alt="Business Logo" class="invoice-logo" />
        </div>`
          : `<div class="company-info">
              <p>${invoiceData["Sender's Zipcode"] || ""}</p>
              <p>${invoiceData["Sender's Address"] || ""}</p>
              <p>${invoiceData["Sender's City"] || ""}</p>
              <p>${invoiceData["Sender's Contact No"] || ""}</p>
            </div>`
      }
    </div>
    ...
    <div class="bill-ship">
      ...
      <div class="ship">
        <h2>Ship To</h2>
        <p>${invoiceData["Receiver's Name"] || ""}</p>
        <p>${invoiceData["Receiver's Address"] || ""}</p>
        <p>${invoiceData["Receiver's City"] || ""}</p>
        <p>${invoiceData["Receiver's Contact No"] || ""}</p>
      </div>
    </div>
