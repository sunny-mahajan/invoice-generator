...
export default function UploadCSV() {
  ...
  // Custom function to handle submission manually
  const onSubmit = () => {
    const data = getValues(); // Get current form values
    trigger().then((isValid) => {
      if (isValid) {
        console.log("Form data:", data); // Proceed with submission
      } else {
        console.log("Validation failed", errors); // Handle validation failure
      }
    });
  };
